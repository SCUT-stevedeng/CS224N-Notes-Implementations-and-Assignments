...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn=nn.Conv1d(embed_size, embed_size,2, padding='same') # a word is treated a as pixel with channel num of embed_size.
        #output tuple shapes: [(Batch_size, seq_length, 2*hidden_size), [(2,Batch_size, hidden_size), (2,Batch_size, hidden_size)]]
        #Btw, Batch_first=True doesn't apply for the shape of c_n and h_n
        ...
        #Project the size of two hidden states(cell states) to one hidden state
        ...
        #This is I think doing a downsampling, projecting the combined output to a lower dimension.
        self.combined_output_projection=nn.Linear(3*self.hidden_size, self.hidden_size, bias=False)
        #output logits for each word in the vocab
        self.target_vocab_projection=nn.Linear(self.hidden_size, len(self.vocab.tgt), bias=False)
        #dropout layer: alleviate over-fitting; function as ensemble learning
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        #Why return log likelihood: Because we are gonna use it as the score for beam search!
        return scores
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X=self.model_embeddings.source(source_padded) #shape:(*, embed_dim)
        X=self.post_embed_cnn(X.permute(1, 2, 0)) #(batch_size, embed_dim, seq_len) corresponding (batch_size, num_channels, length)
        X=X.permute(2, 0, 1)
        packed=pack_padded_sequence(X, source_lengths)
        enc_hiddens_packed, (last_hidden, last_cell)=self.encoder(packed)
        enc_hiddens, lens_unpacked=pad_packed_sequence(enc_hiddens_packed)
        enc_hiddens=enc_hiddens.permute(1, 0, 2) # to (batch_size, seq_len, hidden_size)
        init_decoder_hidden=self.h_projection(torch.concat([last_hidden[0], last_hidden[1]], dim=1))
        init_decoder_cell=self.c_projection(torch.concat([last_cell[0], last_cell[1]], dim=1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj=self.att_projection(enc_hiddens.reshape(-1, enc_hiddens.size(-1))).reshape(enc_hiddens.size(0), enc_hiddens.size(1), -1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t=(enc_hiddens_proj*dec_hidden.unsqueeze(dim=1)).sum(dim=2)
        ...
        alpha_t=F.softmax(e_t, dim=1) #src_seq_len dim
        a_t=(enc_hiddens*alpha_t.unsqueeze(dim=2)).sum(dim=1) #weight every time step and sum on dim seq_len. (batch_size, 2*h)
        U_t=torch.concat([a_t, dec_hidden], dim=1) # concat on dim hidden_size, get(batch_size, 2*h+h)
        V_t=self.combined_output_projection(U_t) # from (b, 3*h) to (b, h)
